...
| `agent-runtime` (Cortex implementation name, pending canonical rename) | Cortex Input Gateway, Control Plane, Scheduler, Workers, low-authority module APIs, Agent Ops API | typed Kernel read/propose API; Research Plane / Research Gateway; model adapters | broker credentials, Kernel writes, official GRACE promotion, Delegation activation |
...
| `research-gateway` (Research Plane connector) | connector sessions, Tool execution, normalization boundary, untrusted-source quarantine; future scheduled collection, temporal evidence/archive and as-of query | approved external research sources; Kernel-published read projections | production Robinhood MCP token/session, any broker mutation, Kernel DB, grant or score writes |
