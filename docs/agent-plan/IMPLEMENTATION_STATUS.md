...
| 数据流与日内循环 | 进行中 | Console 已能通过 Kernel → Cortex → Research Gateway 创建 Provider 数据库持久化、generation-fenced 的 Moody Blues GEX replay，并逐帧显示虚拟市场时间、available time、Spot、Zero Gamma、Call Wall 与 Put Wall；浏览器无 Provider 凭据或 raw Blob 元数据。页面内支持暂停及 1×/10×/60× 自动播放，关闭页面即停止，不冒充服务端后台任务。`moody_blues_replay` 已作为独立模拟 Trigger Source 接入：每帧先生成只含十进制定点字符串的 `gex_compact_v1`，再把 replay ID / generation、原观察摘要、normalize 摘要永久绑定到 Sample；命中后复用 Occurrence → Cortex Wake，页面显示 Wake Run。历史虚拟时间与 Live 五分钟新鲜度隔离，同一帧/Trigger 幂等，验收期间始终 Paper + Observe + Execution Locked，未产生 Authorization / Receipt。下一步是把回放会话、运行中 Run、Candidate 和 Paper 账本组合成完整日内 Session 视图；用户仍可在右侧对话中途参与 |
