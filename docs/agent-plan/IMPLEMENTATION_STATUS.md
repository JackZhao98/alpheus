...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; OpenAI query token input at `9bb3bc8`; minimal durable query dispatcher at `95ac5f2`; read-only Scout -> Decision Desk workflow at `e84eadb`; typed Intent/capability routing at `24f94ff`; deterministic Robinhood research enrichment at `015e9d4`; RSI/MACD/ATR context at `20ce09d`; canonical AP1 Worker integration not started | The default Auto query first runs a typed research-only Intent Interpreter against a code-owned active capability manifest. It may route only to Scout or Team, or refuse; unknown, duplicate, or route-incomplete capability selections fail closed. Manual Team/Scout selection remains available to avoid the extra model call. Team runs a typed Scout evidence brief over normalized quote/bars plus six parallel safe MCP reads: provider-computed daily RSI/MACD/ATR, equity fundamentals, quarterly financials, and symbol earnings results. An unavailable enrichment is explicitly marked rather than failing the whole query. No news source is present, and the Scout prompt forbids inventing one. Decision Desk receives the Scout artifact plus canonical portfolio context for a typed `WAIT/PASS` synthesis. Code rejects `PROPOSE`, non-empty proposals, or a blackboard patch on this path, so it cannot emit an operation. Kernel persists the asynchronous `agent_query_job` lifecycle and sanitized result/error while the manually entered OpenAI token remains page-memory-only. This MVP dispatcher is deliberately not the canonical AP1 Worker; process-crash recovery remains deferred until credential handling is resolved. Effect remains `none`; there is no broker mutation or Live effect. |
