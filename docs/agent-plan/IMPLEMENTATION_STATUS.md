...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; durable integration not started | A synchronous read-only Scout query now crosses Cockpit -> Kernel -> Runtime, receives normalized quote/bars, validates typed output, and cannot emit an operation. Next replace this preview execution with AP1 persistence and bounded Worker claims; no second service, broker access or Live effect. |
