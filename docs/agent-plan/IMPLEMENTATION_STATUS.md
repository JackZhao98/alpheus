...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; OpenAI query token input at `9bb3bc8`; minimal durable query dispatcher at `95ac5f2`; read-only Scout -> Decision Desk workflow at `e84eadb`; typed Intent/capability routing at `24f94ff`; canonical AP1 Worker integration not started | The default Auto query first runs a typed research-only Intent Interpreter against a code-owned active capability manifest. It may route only to Scout or Team, or refuse; unknown, duplicate, or route-incomplete capability selections fail closed. Manual Team/Scout selection remains available to avoid the extra model call. Team runs a typed Scout evidence brief over normalized quote/bars, then gives that artifact plus canonical portfolio context to Decision Desk for a typed `WAIT/PASS` synthesis. Code rejects `PROPOSE`, non-empty proposals, or a blackboard patch on this path, so it cannot emit an operation. Kernel creates a durable `agent_query_job`, transitions it through queued/running/terminal states with compare-and-set updates, dispatches asynchronously, persists the sanitized result or error code, and lets Agent Lab poll by job id. The manually entered OpenAI token remains page-memory-only and does not enable background cognition. This MVP table and goroutine are deliberately not the canonical AP1 Run/Task/Attempt/lease Worker; process-crash recovery remains deferred. Next replace the process-local dispatcher with bounded AP1 Worker claims and recovery after credential handling is resolved. Effect remains `none`; there is no broker access or Live effect. |
