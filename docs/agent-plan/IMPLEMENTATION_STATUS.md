...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; durable integration not started | A synchronous read-only Scout query now crosses the separate Agent Lab -> Kernel -> Runtime, receives normalized quote/bars, validates typed output, and cannot emit an operation. Human access uses a local password and bounded HttpOnly session instead of exposing machine tokens. Next replace this preview execution with AP1 persistence and bounded Worker claims; no second service, broker access or Live effect. |
