...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; OpenAI query token input at `9bb3bc8`; minimal durable query dispatcher at `95ac5f2`; read-only Scout -> Decision Desk workflow at `e84eadb`; canonical AP1 Worker integration not started | The default manual Team query crosses Agent Lab -> Kernel -> Runtime, runs a typed Scout evidence brief over normalized quote/bars, then gives that artifact plus canonical portfolio context to Decision Desk for a typed `WAIT/PASS` synthesis. Code rejects `PROPOSE`, non-empty proposals, or a blackboard patch on this path, so it cannot emit an operation. Scout-only remains available. Kernel creates a durable `agent_query_job`, transitions it through queued/running/terminal states with compare-and-set updates, dispatches asynchronously, persists the sanitized result or error code, and lets Agent Lab poll by job id. Human access uses a local password and bounded HttpOnly session. The manually entered OpenAI token remains page-memory-only, is handed only to the short-lived dispatcher, is never persisted, and does not enable background cognition. This MVP table and goroutine are deliberately not the canonical AP1 Run/Task/Attempt/lease Worker; process-crash recovery remains deferred. Next add a minimal intent/capability routing envelope before replacing the dispatcher with bounded AP1 Worker claims and recovery. Effect remains `none`; there is no broker access or Live effect. |
