...
| Decision Trigger Registry | 已部署 | immutable generation、确定性采样、cross/threshold/cooldown、Occurrence、外部事件 Run/Task/Attempt 唤醒和崩溃恢复均已完成；Trigger Run 可进入 Candidate-aware 并行 TaskGraph。已补齐 fired sample→Occurrence、已入库 Graph→Node Session、外部事件动态 round 三处重启恢复边界。Trigger 暂停或换代会撤销旧 Run Authority，Control 在一个恢复周期内原子关闭整棵执行树、释放预算并以 `runtime_authority_not_current` 写入不可变 Trace；当前 Effect Ceiling 仍为 `none` |
...
| 数据流与日内循环 | 进行中 | Console 已能通过 Kernel → Cortex → Research Gateway 创建 Provider 数据库持久化、generation-fenced 的 Moody Blues GEX replay，并逐帧显示虚拟市场时间、available time、Spot、Zero Gamma、Call Wall 与 Put Wall；浏览器无 Provider 凭据或 raw Blob 元数据。页面内支持暂停及 1×/10×/60× 自动播放，关闭页面即停止，不冒充服务端后台任务。`moody_blues_replay` 已作为独立模拟 Trigger Source 接入：每帧先生成只含十进制定点字符串的 `gex_compact_v1`，再把 replay ID / generation、原观察摘要、normalize 摘要永久绑定到 Sample；命中后复用 Occurrence → Cortex Wake。Kernel migration 0035 新增按用户和 Paper/Live 环境隔离的永久 Intraday Session 投影，把 Replay generation、帧时间和 Wake Run 引用组成 `DATA → TRIGGER → CORTEX → DECISION → EFFECT` 控制台链；刷新页面可从 PostgreSQL 恢复并继续推进，Run 活跃时自动跟踪 Trace、Candidate 和 Effect Receipt。Console 现在默认 Paper，只有用户明确选择才进入 Live，且 Live 仍锁定 Observe。真实 Paper Session `3289a395-1c9a-42fe-979d-ed661833d9bb` 在 generation 3 唤醒 Run `690c6c77-3d2f-4f8f-a56f-52389fb26276` 并跟踪至 succeeded / 11 Trace；本次决策没有 Candidate、Authorization、Receipt 或成交。下一步是把页面内自动播放提升为可恢复的服务端市场 Session 调度，并加入 Session 级暂停、结束和策略组合控制；用户仍可在右侧对话中途参与 |
...
| AP1-2 PostgreSQL durable state and command transactions | In progress; immutable definitions at `bce88cc`; durable Runtime state at `7671762`; claim/start/heartbeat commands at `95a1af2`; model-call transactions at `4f3a082`; Attempt terminalization at `9ea1c04`; bounded output validator contracts at `f70388d`; root admission, child/TaskGraph admission, output-validation evidence, Run cancellation and revoked-authority reconciliation deployed | OwnerPolicy, RuntimePolicy, JSON OutputContract, Run/Task/Session/Attempt/Turn, model-call, Artifact, Checkpoint, budget, cancellation, recovery, idempotency-record, and transition-event state are durable, exact-lineage-bound, default-deny, and effect `none`. Cortex uses separate Activator, Control, and Worker LOGINS. Control atomically admits exact-current-policy execution trees, validates model output against the exact committed schema, and reconciles authenticated cancellation or stale authority across Turn, Attempt, Session, Task, graph schedule and Run resources. Unknown provider outcomes remain `canceling` until their existing reconciliation path produces a known result. Formal standalone Result-linked validation receipts and the broader AP1 stage seal remain deferred. |
