...
- AP1-2's immutable definitions landed at `bce88cc`, its default-deny durable
  ...
  AP1 path yet; child-task, cancellation, and recovery commands remain absent.
  ...
  persistence is intentionally deferred until after the MVP loop. This
  canonical AP1 path cannot call a model or produce an external effect.
...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; minimal durable query dispatcher at `95ac5f2`; read-only Scout -> Decision Desk workflow at `e84eadb`; typed Intent/capability routing at `24f94ff`; deterministic Robinhood research enrichment at `015e9d4`; RSI/MACD/ATR context at `20ce09d`; encrypted database credentials at `5759e04`; isolated Robinhood news gateway at `2d6aa00`; canonical AP1 Worker integration not started | The default Auto query first runs a typed research-only Intent Interpreter against a code-owned active capability manifest. It may route only to Scout or Team, or refuse; unknown, duplicate, or route-incomplete capability selections fail closed. Manual Team/Scout selection remains available to avoid the extra model call. Team runs a typed Scout evidence brief over normalized quote/bars plus seven concurrent read enrichments: provider-computed daily RSI/MACD/ATR, equity fundamentals, quarterly financials, symbol earnings results, and normalized Robinhood news headlines. An unavailable enrichment is explicitly marked rather than failing the whole query. Headlines are dated third-party claims, not execution truth or proof of the underlying claim. Decision Desk receives the Scout artifact plus canonical portfolio context for a typed `WAIT/PASS` synthesis. Code rejects `PROPOSE`, non-empty proposals, or a blackboard patch on this path, so it cannot emit an operation. Kernel persists the asynchronous `agent_query_job` lifecycle and sanitized result/error. OpenAI, future Brave, and the separate read-only Robinhood research credential use provider-bound AES-256-GCM envelopes in PostgreSQL; the wrapping key is independently derived from the existing Agent Web session key. Browser requests, jobs, events, responses and Agent Runtime environment contain no persisted plaintext key. Rotating the session key intentionally makes old envelopes unreadable until the credentials are replaced. This MVP dispatcher is deliberately not the canonical AP1 Worker; process-crash recovery remains deferred. Effect remains `none`; there is no broker mutation or Live effect. |
...
The existing Tofi Brave/DuckDuckGo search and Chrome/HTTP fetch implementations
were reviewed but not copied. Their provider and extraction behavior is useful,
but Alpheus still needs typed connectors, bounded responses, source metadata,
redirect validation, private-network/DNS-rebinding denial, and explicit
degraded behavior before Web Search or Web Fetch is enabled. That is the next
research-data MVP slice.
