...
| AP1-2 PostgreSQL durable state and command transactions | In progress; AP0 historical certification scope frozen at `714bee2` | Runtime policy/state tables, narrow role-derived command functions, idempotency, generation and lease fencing, atomic budget accounting, cancellation and recovery. |
