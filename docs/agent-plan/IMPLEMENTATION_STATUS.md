...
| AP1-3 Control Plane and bounded Worker execution | MVP preview at `dd55a30`; password-protected Agent Lab at `04d0139`; OpenAI query token input at `9bb3bc8`; minimal durable query dispatcher at `95ac5f2`; canonical AP1 Worker integration not started | A read-only Scout query crosses Agent Lab -> Kernel -> Runtime, receives normalized quote/bars, validates typed output, and cannot emit an operation. Kernel now creates a durable `agent_query_job`, transitions it through queued/running/terminal states with compare-and-set updates, dispatches the model call asynchronously, persists the sanitized result or error code, and lets Agent Lab poll by job id. Human access uses a local password and bounded HttpOnly session. A manually entered OpenAI token remains page-memory-only, is handed only to that short-lived dispatcher, is never persisted, and does not enable background cognition. This MVP table and goroutine are deliberately not the canonical AP1 Run/Task/Attempt/lease Worker; process-crash recovery remains deferred. Next replace the dispatcher with bounded AP1 Worker claims and recovery while retaining effect `none`; no broker access or Live effect. |
