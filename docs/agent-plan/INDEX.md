...
| User query, Intent, interruption, and confirmation | Architecture frozen; immutable Conversation/UserRequest contracts and default-deny storage are implemented, while the sole Input Gateway submission command, Intent/PolicyResolution, UI transport and all confirmation state remain required; trading money-confirmation subset is frozen by Delegation | [`USER_INPUT.md`](USER_INPUT.md), [`DELEGATION_POLICY.md`](DELEGATION_POLICY.md) |
