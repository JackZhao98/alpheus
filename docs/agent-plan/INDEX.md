...
| Module | Status | File |
|---|---|---|
| Agent Platform implementation tracker | AP0-1 through AP0-6 accepted; AP1-2 durable foundations are in place; the read-only Agent Lab -> Kernel -> Scout path now has a minimal asynchronous persisted query lifecycle at `95ac5f2`, while canonical AP1 Worker/lease/recovery wiring remains next and all effects remain closed | [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) |
