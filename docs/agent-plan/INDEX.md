...
| Module | Status | File |
|---|---|---|
| Agent Platform implementation tracker | AP0-1 through AP0-6 accepted; AP1-2 durable foundations are in place; Agent Lab now has typed Auto intent/capability routing over its asynchronous persisted read-only Scout -> Decision Desk workflow at `24f94ff`; canonical AP1 Worker/lease/recovery wiring remains next and all effects remain closed | [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) |
