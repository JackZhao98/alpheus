...
| Module | Status | File |
|---|---|---|
| Agent Platform implementation tracker | AP0-1 through AP0-6 accepted; AP1-2 durable foundations are in place; Agent Lab now runs an asynchronous persisted read-only Scout -> Decision Desk query workflow at `e84eadb`, while intent/capability routing and canonical AP1 Worker/lease/recovery wiring remain next and all effects remain closed | [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) |
