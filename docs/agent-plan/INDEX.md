...
| Module | Status | File |
|---|---|---|
| Agent Platform implementation tracker | AP0-1 through AP0-6 accepted. The 10-step Cortex cutover is deployed locally: Agent Lab submits canonical UserRequests, a credential-isolated Worker persists Attempt/Turn/Artifact state around verified `gpt-5.6-sol` calls, and the page no longer uses the deprecated Kernel query queue. Research evidence persistence, broader recovery, and the real Brave provider smoke remain open while all effects stay closed | [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) |
...
| User query, Intent, interruption, and confirmation | Architecture frozen; immutable Conversation/UserRequest contracts, default-deny storage, and scoped idempotent database admission are implemented. The browser-facing Input Gateway, blob staging, Intent/PolicyResolution, UI transport and all confirmation state remain required; trading money-confirmation subset is frozen by Delegation | [`USER_INPUT.md`](USER_INPUT.md), [`DELEGATION_POLICY.md`](DELEGATION_POLICY.md) |
...
| Cortex Tool Registry tracker | Current inventory: 36 enabled read/read-preflight Tools with receipt paths; all 15 Robinhood writes remain explicitly excluded | [`TOOL_REGISTRY_TRACKER.md`](TOOL_REGISTRY_TRACKER.md) |
| Cortex + Research launch tracker | Chinese operator-facing launch checklist for Cortex execution, Research Gateway, Moody Blues and Provider verification; trading execution remains out of scope | [`CORTEX_RESEARCH_LAUNCH_TRACKER.md`](CORTEX_RESEARCH_LAUNCH_TRACKER.md) |
