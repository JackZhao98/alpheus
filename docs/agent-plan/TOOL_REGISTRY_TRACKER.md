...
- **归属 Agent**：Control 强制执行的唯一专业角色授予。普通只读 Tool 必须
  先交给该 Specialist 形成持久化 memo，再回到 Decision Desk；两条订单
  review 预检仍由 Decision Desk 独占。

## 总进度

| 工作流 | 总数 | 已注册 | 已启用给 Cortex | 状态 |
|---|---:|---:|---:|---|
| Cortex 跨平面只读工具 | 37 | 37 | 37 | 全部已启用；Agent Lab 显示 37 已启用 / 0 待接入 |
...
| R8 | Trace 与网页证据展示 | 显示工具授权、执行收据、来源和时间 | **已完成**：Agent Lab 阶段 A 显示 37/37 已启用，阶段 B 单独验证自主意图路线；5 个 UUID 型测试会先要求填入前置只读工具返回的真实 ID |
| R9 | 能力与角色授予验收 | 未获授权的 Worker 无法直接调用上游 MCP | **已完成**：6 个专业 Agent 均已部署；Control 在每次 Tool 授权时强制检查唯一角色 grant，Intent 每次最多提出 1 个 Tool，Desk 只消费收据与 Specialist memo |

## 当前已启用的 Cortex 工具

| Tool ID | 入口端口与路径 | Provider 路径 | 描述 | 当前 Agent 范围 | 状态 |
|---|---|---|---|---|---|
| `research_web_fetch` | Cortex Input `:8400` `/internal/v1/tool-calls/web-fetch` | Research Gateway `:8300` `/internal/v1/cortex-tools/web-fetch` | 抓取用户不可变文本中一个明确的公开 HTTP(S) URL，返回有界且不可信的页面证据；它不是搜索。 | Discovery Scout → Decision Desk | 已启用 |
| `research_gexbot_as_of` | Cortex Input `:8400` `/internal/v1/tool-calls/gexbot-as-of` | Research Gateway `:8300` → GEXBOT Provider `:8500` | 按时间围栏读取一个 SPX GEX 历史序列，返回标准化历史证据。 | Options Scout → Decision Desk | 已启用 |
| `market_gexbot_live` | Cortex Input `:8400` `/internal/v1/tool-calls/gexbot-live` | Research Gateway `:8300` → GEXBOT Provider `:8500 /v1/live` | 按需读取一条官方 SPX GEX API 响应并永久保存 raw Blob、来源时间、抓取时间和收据；不是历史回放。 | Options Scout → Decision Desk | 已启用；真实收据已验证 |
| `kernel_earnings_results` | Cortex Input `:8400` `/internal/v1/tool-calls/kernel-earnings-results` | Kernel `:8100` `/internal/v1/cortex-tools/earnings-results` → Robinhood MCP `get_earnings_results` | 只读取一个明确股票代码的标准化财报结果：EPS estimate / actual、报告日期、盘前/盘后和确认状态；丢弃上游 guide、通用 MCP 参数、账户和凭据。 | Catalyst Scout → Decision Desk | 已启用；真实收据已验证 |
...
| `get_earnings_calendar` | 催化剂 | 指定股票近期财报日期 | Catalyst Scout | Cortex 已启用 |
| `get_earnings_results` | 催化剂 | 指定股票已发布财报结果 | Catalyst Scout | 已通过窄桥接对 Cortex 启用 |
...
| `get_equity_tradability` | 市场 | 可交易性和市场状态事实 | Market Scout | Cortex 已启用 |
...
| `get_popular_watchlists` | 发现 | 公开热门自选列表元数据 | Discovery Scout | Cortex 已启用 |
| `get_watchlists` | 发现 | 已绑定账户或公开自选列表元数据 | Discovery Scout | Cortex 已启用 |
| `get_watchlist_items` | 发现 | 一个明确自选列表 ID 的内容 | Discovery Scout | Cortex 已启用 |
| `get_scanner_filter_specs` | 发现 | 有效的 Scanner filter 定义 | Discovery Scout | Cortex 已启用 |
| `get_scans` | 发现 | 可用 Scanner 定义 | Discovery Scout | Cortex 已启用 |
| `run_scan` | 发现 | 执行一个获准且有界的 Scanner ID | Discovery Scout | Cortex 已启用 |
| `search` | 发现 | 把资产名称/股票代码解析为 provider ID | Discovery Scout | Cortex 已启用 |
...
| 服务 | 端口 | 职责 | 是否承载 Agent |
|---|---|---|---|
| Kernel | `0.0.0.0:8100` | Agent Lab、Robinhood 只读桥、交易边界；不再发起旧 LLM query | 否 |
| Cortex Input / Control | `127.0.0.1:8400` | UserRequest、Run、handoff、Tool 授权、收据确认与查询投影 | Intent / Control |
| Cortex Worker | 无监听端口 | 执行 LLM Turn、专业 Agent memo 与 Decision Desk | 6 个 Specialist + Decision Desk |
| Research Gateway | Docker 内部 `:8300` | Web/GEX connector、Moody Blues、证据标准化与收据 | 否 |
| GEXBOT Provider | `127.0.0.1:8500` | 官方采集、raw Blob、历史 `as_of` 与 replay | 否 |
| PostgreSQL | Docker 内部 `:5432` | 不可变记录、授权、Trace、历史数据 | 否 |

迁移、LOGIN provisioning、密钥导出容器均为一次性任务，运行完退出，不占用
常驻 CPU/内存。旧 `agent-runtime:8200` 已从 Compose 移除。
...
|---|---|---|---|---|---|
| `research_web_fetch` | `:8400 → :8300` | Research `web-fetch` | 读取一个明确公开 URL 的有界、不可信证据 | Discovery Scout | 已启用 |
| `research_gexbot_as_of` | `:8400 → :8300 → :8500` | GEXBOT Provider archive | 按 `as_of` 围栏读取 SPX GEX 历史快照 | Options Scout | 已启用 |
| `market_gexbot_live` | `:8400 → :8300 → :8500` | GEXBOT 官方 API | 按需读取官方 SPX GEX，保存来源/抓取时间、raw Blob 与收据 | Options Scout | 已启用 |
| `kernel_accounts` | `:8400 → :8100` | `get_accounts` | 绑定账户的身份、类型与状态 | Position Manager | 已启用 |
| `kernel_earnings_calendar` | `:8400 → :8100` | `get_earnings_calendar` | 有界的近期财报日历 | Catalyst Scout | 已启用 |
| `kernel_earnings_results` | `:8400 → :8100`（专用桥） | `get_earnings_results` | 标准化 EPS 与报告日期事实 | Catalyst Scout | 已启用 |
| `kernel_equity_fundamentals` | `:8400 → :8100` | `get_equity_fundamentals` | 股票基本面与估值字段 | Fundamental Scout | 已启用 |
| `kernel_financials` | `:8400 → :8100` | `get_financials` | 有界财务报表数据 | Fundamental Scout | 已启用 |
| `kernel_equity_historicals` | `:8400 → :8100` | `get_equity_historicals` | 有界历史股价 K 线 | Market Scout | 已启用 |
| `kernel_equity_price_book` | `:8400 → :8100` | `get_equity_price_book` | bid、ask 与盘口快照 | Market Scout | 已启用 |
| `kernel_equity_quotes` | `:8400 → :8100` | `get_equity_quotes` | 股票报价快照 | Market Scout → Desk | 已启用 |
| `kernel_equity_technical_indicators` | `:8400 → :8100` | `get_equity_technical_indicators` | 指定区间的单一技术指标 | Market Scout | 已启用 |
| `kernel_equity_tradability` | `:8400 → :8100` | `get_equity_tradability` | 可交易性与市场状态 | Market Scout | 已启用 |
| `kernel_indexes` | `:8400 → :8100` | `get_indexes` | 指数 symbol 与 Provider ID 解析 | Market Scout | 已启用 |
| `kernel_index_quotes` | `:8400 → :8100` | `get_index_quotes` | 指数报价快照 | Market Scout | 已启用 |
| `kernel_option_chains` | `:8400 → :8100` | `get_option_chains` | 标的期权链元数据 | Options Scout | 已启用 |
| `kernel_option_instruments` | `:8400 → :8100` | `get_option_instruments` | 期权合约 ID 与条款 | Options Scout | 已启用 |
| `kernel_option_quotes` | `:8400 → :8100` | `get_option_quotes` | 指定期权合约报价 | Options Scout | 已启用 |
| `kernel_option_watchlist` | `:8400 → :8100` | `get_option_watchlist` | 现有期权自选列表快照 | Options Scout | 已启用 |
| `kernel_option_level_upgrade_info` | `:8400 → :8100` | `get_option_level_upgrade_info` | 绑定账户的期权资格 | Position Manager | 已启用 |
| `kernel_equity_positions` | `:8400 → :8100` | `get_equity_positions` | 绑定账户股票持仓 | Position Manager | 已启用 |
| `kernel_option_positions` | `:8400 → :8100` | `get_option_positions` | 绑定账户期权持仓 | Position Manager | 已启用 |
| `kernel_equity_orders` | `:8400 → :8100` | `get_equity_orders` | 股票订单历史与状态，只读 | Position Manager | 已启用 |
| `kernel_option_orders` | `:8400 → :8100` | `get_option_orders` | 期权订单历史与状态，只读 | Position Manager | 已启用 |
| `kernel_equity_tax_lots` | `:8400 → :8100` | `get_equity_tax_lots` | 股票 tax lots | Position Manager | 已启用 |
| `kernel_portfolio` | `:8400 → :8100` | `get_portfolio` | 绑定账户组合汇总 | Position Manager → Desk | 已启用 |
| `kernel_pnl_trade_history` | `:8400 → :8100` | `get_pnl_trade_history` | 有界已实现交易 P&L 历史 | Position Manager | 已启用 |
| `kernel_realized_pnl` | `:8400 → :8100` | `get_realized_pnl` | 有界已实现 P&L 汇总 | Position Manager | 已启用 |
| `kernel_popular_watchlists` | `:8400 → :8100` | `get_popular_watchlists` | 公开热门自选列表元数据 | Discovery Scout | 已启用 |
| `kernel_watchlists` | `:8400 → :8100` | `get_watchlists` | 自选列表名称与 ID | Discovery Scout | 已启用 |
| `kernel_watchlist_items` | `:8400 → :8100` | `get_watchlist_items` | 指定自选列表 ID 的内容 | Discovery Scout | 已启用 |
| `kernel_scanner_filter_specs` | `:8400 → :8100` | `get_scanner_filter_specs` | Scanner 筛选字段定义 | Discovery Scout | 已启用 |
| `kernel_scans` | `:8400 → :8100` | `get_scans` | 可用 Scanner 定义 | Discovery Scout | 已启用 |
| `kernel_run_scan` | `:8400 → :8100` | `run_scan` | 执行一个明确的 Scanner ID | Discovery Scout | 已启用 |
| `kernel_search` | `:8400 → :8100` | `search` | 名称或代码到 Provider ID 的解析 | Discovery Scout | 已启用 |
