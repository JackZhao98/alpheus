...
>
> Current implementation target: **K1B runtime policy binding and absolute
> database time. M11 is
...
| X — Policy ownership | M11 K0; K1; Agent K2 | **K0 LANDED; K1 IN PROGRESS (K1A LANDED)** | [`06_POLICY_OWNERSHIP.md`](06_POLICY_OWNERSHIP.md) |
...
| **K1** | **IN PROGRESS** | M11 non-money gate; before AP0; real Canary not required; zero production broker mutation | K1A `229a77b`: typed strict policy schema, dead-field-removing YAML import, canonical versioned digest, immutable revisions, CAS head, audited conflicts and explicit idempotent CLI; isolated PostgreSQL/race/vet green. K1B revision binding, database expiry/lease and YAML runtime removal remain; K1C completed-day attestation/guarded widening remains | Phase X |
