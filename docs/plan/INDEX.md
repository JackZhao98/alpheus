...
| **M11** | **BLOCKED** | M10; provider must expose stable client-id lookup/fill identity | fail-closed live canary with 20-way memory and PostgreSQL one-allowance barriers; failed attempts burn allowance; first live grant requires exactly one provider quantity increment; Class-C approval recheck; fixed account-bound/no-cache/single-attempt MCP mutation transport; dormant adapter requires an unavailable `ClientOrderLookup`, revalidates exact instrument increments/option effect/provider echo and matches FakeBroker replay; live startup refuses unset canary; full pre-live certification green; `docs/M11_PROVIDER_GAP.md` records the provider no-ship gap | Phase 4 |
