...
> Plan version: **v1.5 — frozen baseline plus evidence amendment**
>
...
| **M11** | **IN PROGRESS** | M10; v1.5 equity recovery is implemented offline, but equity limit tick/increment metadata is not provider-documented; option mutations remain blocked | `0bf183a`; fail-closed live canary and immutable revision ledger; structured sanitized mutation errors; migration 0009 durable account-level active/unknown latch; pre-send account/canonical-intent/fingerprint/time-window evidence; one byte-identical same-ref equity replay maximum; exact paged candidate matching; zero/multiple candidates stay latched; sole candidate is two-step Admin-gated and re-pulled before transactional adoption; candidate fault matrix and 20-way confirmation race retain latch/resources on every failure; PostgreSQL adoption rollback remains reclaimable without clearing unknown, while the success path atomically commits broker identity/fill/exposure/reservation and clears the latch; 20-way PostgreSQL gate race, full store race suite, kernel/runtime race+vet, and clean-volume isolated Compose smoke green; owner-authorized equity A/B evidence recorded; option negative evidence recorded; production execution remains unwired with zero new real-money probes | Phase 4 |
