...
> Plan version: **v1.9.7 — opt-in Robinhood equity auto-replay**
>
...
| 2026-07-20 | v1.9.7 | Add an opt-in activation path for Robinhood equity same-ref auto-replay | The same-ref replay recovery for an unknown place (reconciler, dedupe-verified, replay-once, send-window bounded) was built and enabled only for FakeBroker; amendment v1.7.1 held it fail-closed for Robinhood pending a certified order-creation-latency bound. The mechanism now takes an explicit replay guard = broker call timeout + `REPLAY_CREATION_GUARD_MS` (default 15s), sized to exceed the provider's observed creation latency so a replayed order's `created_at` cannot fall outside the send window and orphan. Activation is a deliberate per-deployment opt-in (`ROBINHOOD_EQUITY_AUTO_REPLAY=true`, live only, equity only); default stays disabled, so this amendment changes availability, not the shipped behavior. This does not itself certify the latency bound — the operator owns the guard value against observed Robinhood creation latency — and does not enable options, market orders, or Agent Live. |
