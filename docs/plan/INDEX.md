...
| **M11** | **BLOCKED** | M10; provider must expose stable client-id lookup/fill identity | fail-closed live canary with 20-way memory and PostgreSQL one-allowance barriers; failed attempts burn allowance; Class-C approval recheck; fixed account-bound/no-cache/single-attempt MCP mutation transport; live startup refuses unset canary; full pre-live certification green; `docs/M11_PROVIDER_GAP.md` records the provider no-ship gap | Phase 4 |
