...
| **M11** | **IN PROGRESS** | M10; v1.6 equity limit contract and live-only Provider wiring complete; isolated live-mode startup certification and a separately confirmed one-share Alpheus canary remain; option mutations blocked | `319f657`; exact-symbol equity instrument identity; certified whole-share limit quantity; $0.01 tick above $1 and $0.0001 at/below $1; canonical post-placement order read; production constructor equity-only; fail-closed canary and immutable revision ledger; migration 0009 durable account active/unknown latch; exact paged candidate matching; single same-ref replay maximum; two-step Admin adoption; candidate fault matrix, 20-way confirmation/gate races, PostgreSQL rollback and positive atomic-adoption proofs; full race/vet green; owner-confirmed live evidence order cancelled with zero fill; deployed stack remains read-only | Phase 4 |
