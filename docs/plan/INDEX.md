...
> Plan version: **v1.9.4 — M11 execution-core acceptance reopened**
>
> Semantic baseline: commit `fa5a29e` (`docs: harden roadmap execution invariants`)
>
> Frozen on: 2026-07-16
>
> Current implementation target: **repair and recertify the minimal real
> Provider execution lifecycle. M11 is `IN PROGRESS`; production remains
> read-only with global Halt committed, and AP13, Option Live, and autonomous
> broker effects remain closed**
...
| 4 — Pre-live and live | M9, M10, M11 | **M11 IN PROGRESS; Agent Live blocked** | [`05_PRELIVE_AND_LIVE.md`](05_PRELIVE_AND_LIVE.md) |
...
| Z — Production evidence | M11 Canary | **IN PROGRESS; v1.9.3 acceptance reopened** | [`08_DEFERRED_CANARY.md`](08_DEFERRED_CANARY.md) |
...
| **M11** | **IN PROGRESS** | v1.9.3 Canary evidence remains valid but did not cover a working explicit close limit across repricer and read-only restart; complete deterministic submit/query/cancel/fill/expiry lifecycle matrix before relanding; AP13 and options blocked | Market fill price-backfill fixed in `2d1b66b`. Owner-confirmed close operation `552b1515-c1a1-48bb-9d32-f2b03d59461b` exposed implicit same-price cancel/replace churn and a read-only typed-nil Broker panic while one order was working. No fill or duplicate simultaneous order occurred. `776635a` makes explicit close limits static and fixes read-only construction. `26a93f2` makes all open/close execution static by default, requires explicit managed Limit opt-in, binds that choice into idempotency, and adds a recorded one-call Robinhood working-sell fixture. Full lifecycle recertification remains. Production returned to `read_only` with global Halt committed | Phase 4 / Z |
