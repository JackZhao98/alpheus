-- A new immutable workflow contract adds one narrowly scoped Kernel fact Tool.
-- Older Runs retain v2-v4 and therefore cannot gain this Tool on retry.
SET ROLE alpheus_agent_migrator;
SET TIME ZONE 'UTC';

CREATE FUNCTION agent_control.ensure_cortex_workflow_output_contract_v5(p_schema JSONB)
...
BEGIN
  ...
  IF invoker.group_role::TEXT<>'alpheus_agent_control_api' OR invoker.profile_id<>'control-api' OR invoker.owner_id<>'agent_control'
    ...
    RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='cortex workflow v5 output contract denied';
  END IF;
  body:=jsonb_build_object('schema_revision',1,'revision_id','cortex-workflow-output-v5','generation',1,'artifact_type','assistant_response','schema',p_schema,
  ...
  INSERT INTO agent_control.output_contract_revision(
    ...
    'cortex-workflow-output-v5',1,1,contract_digest,'assistant_response',1,(p_schema->>'blob_id')::UUID,
  ...
  IF NOT EXISTS (SELECT 1 FROM agent_control.output_contract_revision
      WHERE revision_id='cortex-workflow-output-v5' AND schema_blob_id=(p_schema->>'blob_id')::UUID) THEN
    RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='cortex workflow v5 output contract identity conflict';
  END IF;
  SELECT record_digest INTO contract_digest FROM agent_control.output_contract_revision WHERE revision_id='cortex-workflow-output-v5';
...
BEGIN
  SELECT pg_get_functiondef('agent_control.admit_cortex_user_request_run_v5(jsonb)'::regprocedure) INTO definition;
  IF position('cortex-workflow-output-v4' IN definition)=0 THEN
    RAISE EXCEPTION 'expected Cortex workflow admission v5 definition';
  END IF;
  EXECUTE replace(replace(definition,'admit_cortex_user_request_run_v5','admit_cortex_user_request_run_v6'),
...
BEGIN
  ...
  IF position('''gexbot_enabled'',selected.gexbot_enabled,''objective''' IN definition)=0
     OR position('cortex-workflow-output-v4' IN definition)=0 THEN
    RAISE EXCEPTION 'expected Cortex Worker discovery definition';
  END IF;
  definition:=replace(definition,
    'CASE WHEN task.output_contract_revision_id IN (''cortex-workflow-output-v3'',''cortex-workflow-output-v4'') THEN true ELSE false END AS scout_enabled, CASE WHEN task.output_contract_revision_id=''cortex-workflow-output-v4'' THEN true ELSE false END AS gexbot_enabled,',
  ...
  definition:=replace(definition,'''gexbot_enabled'',selected.gexbot_enabled,''objective''','''gexbot_enabled'',selected.gexbot_enabled,''earnings_enabled'',selected.earnings_enabled,''objective''');
  IF position('earnings_enabled' IN definition)=0 THEN
    RAISE EXCEPTION 'Cortex Worker earnings discovery replacement failed';
  END IF;
  EXECUTE definition;
END $$;

REVOKE ALL ON FUNCTION agent_control.ensure_cortex_workflow_output_contract_v5(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION agent_control.admit_cortex_user_request_run_v6(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION agent_control.next_cortex_task() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION agent_control.ensure_cortex_workflow_output_contract_v5(JSONB) TO alpheus_agent_control_api;
GRANT EXECUTE ON FUNCTION agent_control.admit_cortex_user_request_run_v6(JSONB) TO alpheus_agent_control_api;
