-- Recover the gap between a committed fired sample and TriggerOccurrence
-- materialization. A crash or transient database failure must not make the
-- first firing disappear merely because later samples enter cooldown.
SET ROLE alpheus_agent_migrator;
SET TIME ZONE 'UTC';

CREATE FUNCTION
agent_control.list_pending_cortex_decision_trigger_occurrences(
...
BEGIN
  ...
  IF invoker.group_role::TEXT<>'alpheus_agent_control_api'
    ...
    RAISE EXCEPTION USING ERRCODE='42501',
      MESSAGE='Cortex decision Trigger occurrence recovery denied';
  END IF;
  RETURN COALESCE((
    SELECT jsonb_agg(
      item.payload ORDER BY item.committed_at,item.sample_id
    )
    FROM (
      SELECT
        sample.committed_at,
      ...
      JOIN agent_control.cortex_decision_trigger_revision AS trigger_row
        ON trigger_row.trigger_id=sample.trigger_id
       AND trigger_row.generation=sample.trigger_generation
       AND trigger_row.record_digest=sample.trigger_digest
      LEFT JOIN agent_control.cortex_decision_trigger_occurrence
        AS occurrence_link
        ON occurrence_link.sample_id=sample.sample_id
      WHERE sample.fired
        AND occurrence_link.sample_id IS NULL
        AND trigger_row.subject_principal_id=p_subject_principal_id
        AND sample.committed_at>
          clock_timestamp()-interval '12 minutes'
      ORDER BY sample.committed_at,sample.sample_id
      LIMIT p_limit
    ) AS item
  ),'[]'::JSONB);
END
