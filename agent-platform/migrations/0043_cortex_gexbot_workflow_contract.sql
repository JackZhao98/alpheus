-- A new closed workflow contract gives the Intent Interpreter one bounded
-- choice: request an archived GEXBOT as_of observation, or do not.  Existing
-- v2/v3 Runs retain their exact schemas and cannot silently acquire a Tool.
SET ROLE alpheus_agent_migrator;
SET TIME ZONE 'UTC';

CREATE FUNCTION agent_control.ensure_cortex_workflow_output_contract_v4(p_schema JSONB)
...
BEGIN
  ...
  IF invoker.group_role::TEXT<>'alpheus_agent_control_api' OR invoker.profile_id<>'control-api' OR invoker.owner_id<>'agent_control'
    ...
    RAISE EXCEPTION USING ERRCODE='42501',MESSAGE='cortex workflow v4 output contract denied';
  END IF;
  body:=jsonb_build_object('schema_revision',1,'revision_id','cortex-workflow-output-v4','generation',1,'artifact_type','assistant_response','schema',p_schema,
  ...
  INSERT INTO agent_control.output_contract_revision(
    ...
    'cortex-workflow-output-v4',1,1,contract_digest,'assistant_response',1,(p_schema->>'blob_id')::UUID,
  ...
  IF NOT EXISTS (SELECT 1 FROM agent_control.output_contract_revision
      WHERE revision_id='cortex-workflow-output-v4' AND schema_blob_id=(p_schema->>'blob_id')::UUID) THEN
    RAISE EXCEPTION USING ERRCODE='23505',MESSAGE='cortex workflow v4 output contract identity conflict';
  END IF;
  SELECT record_digest INTO contract_digest FROM agent_control.output_contract_revision WHERE revision_id='cortex-workflow-output-v4';
...
BEGIN
  SELECT pg_get_functiondef('agent_control.admit_cortex_user_request_run_v4(jsonb)'::regprocedure) INTO definition;
  IF position('cortex-workflow-output-v3' IN definition)=0 THEN
    RAISE EXCEPTION 'expected Cortex workflow admission v4 definition';
  END IF;
  EXECUTE replace(replace(definition,'admit_cortex_user_request_run_v4','admit_cortex_user_request_run_v5'),
...
BEGIN
  ...
  IF position('cortex-workflow-output-v3' IN definition)=0 OR position('''scout_enabled'',selected.scout_enabled,''objective''' IN definition)=0 THEN
  ...
  definition:=replace(definition,'''scout_enabled'',selected.scout_enabled,''objective''','''scout_enabled'',selected.scout_enabled,''gexbot_enabled'',selected.gexbot_enabled,''objective''');
  EXECUTE definition;
END $$;

REVOKE ALL ON FUNCTION agent_control.ensure_cortex_workflow_output_contract_v4(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION agent_control.admit_cortex_user_request_run_v5(JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION agent_control.next_cortex_task() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION agent_control.ensure_cortex_workflow_output_contract_v4(JSONB) TO alpheus_agent_control_api;
GRANT EXECUTE ON FUNCTION agent_control.admit_cortex_user_request_run_v5(JSONB) TO alpheus_agent_control_api;
