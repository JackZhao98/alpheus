-- Permit the proposal contract for a follow-up round only when the current
-- root Attempt belongs to the exact continuation Session authorized by the
-- prior Decision Desk. The root Task's final Artifact contract stays frozen.
  ...
  SELECT
    ...
    OR EXISTS (
      ...
      JOIN agent_control.runtime_task AS task
       ...
       AND task.parent_task_id IS NULL
       AND task.output_contract_revision_id='cortex-workflow-output-v8'
      JOIN agent_control.cortex_task_graph_round_continuation AS continuation
        ON continuation.parent_task_id=task.task_id
       AND continuation.run_id=task.run_id
       AND continuation.parent_session_id=attempt.session_id
       AND continuation.state='ready'
       AND continuation.completed_round < continuation.max_rounds
      JOIN agent_control.output_contract_revision AS proposal_contract
        ON proposal_contract.revision_id=
          'cortex-task-graph-proposal-output-v1'
       AND proposal_contract.generation=1
