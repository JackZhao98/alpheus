-- Contract installation must replay the stored immutable revision rather than
-- recomputing a new created_at-dependent digest on every Control restart.
SET ROLE alpheus_agent_migrator;
SET TIME ZONE 'UTC';

CREATE OR REPLACE FUNCTION
agent_control.ensure_cortex_task_graph_round_output_contract_v1(
  p_schema JSONB
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  IF FOUND THEN
    IF existing.generation<>1
       OR existing.schema_blob_id<>(p_schema->>'blob_id')::UUID
       OR existing.schema_blob_content_digest<>p_schema->>'content_digest'
       OR existing.schema_origin_record_digest<>
          p_schema#>>'{origin,record_digest}'
       OR existing.effect_class<>'none' THEN
