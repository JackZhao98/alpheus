-- A round decision is stored in the model output Blob. Preserve that Blob's
-- immutable origin when granting the follow-up planner session read access.
...
$old$;
    new_fragment TEXT:=$new$
      (result_row.output->>'blob_id')::UUID,
      result_row.output#>>'{origin,record_type}',
      result_row.output#>>'{origin,record_id}',
      result_row.output#>>'{origin,record_digest}',
...
BEGIN
    ...
    IF definition=original_definition
       OR position(new_fragment IN definition)=0 THEN
        RAISE EXCEPTION 'Unexpected TaskGraph round Blob origin definition';
    END IF;
    EXECUTE definition;
END
$migration$;
