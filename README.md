...
- [`docs/PLAN.md`](docs/PLAN.md) — 按里程碑执行的实现计划。
...
`go test ./...` 可跑；这是 `docs/PLAN.md` Milestone 9 风险测试扩展的种子。
...
- **A 减风险**（平仓/撤单/收紧止损）：零审批即刻执行——止损路径只有一跳。
  平仓默认使用可成交价（平多卖 bid、平空买 ask）；撤单按
...
只读接入（见 `docs/PLAN.md` Milestone 8）、inbox/watchlist 注入（assemble 有
