...
M10 的 cognition transport 已接入官方 Anthropic Go SDK，但 prompt 槽位仍按
冻结计划保持空白，默认也仍是 `COGNITION=stub`。启用 `llm` 时必须在启动前
提供 `ANTHROPIC_API_KEY`、`DECIDER_MODEL` 和 `MONITOR_MODEL`；每个槽位先受
