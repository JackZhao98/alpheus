...
`live` 还必须同时满足 `LIVE_TRADING_ENABLED=true`、已在 Kernel 数据库中加密
保存的 Robinhood 连接，以及用户明确确认的 Agentic 账户绑定；且拒绝 FakeBroker。
...
使用 `BROKER=robinhood` 时，到 `/agent-lab` 点击 **Connect Robinhood**。Kernel
发起动态 OAuth + PKCE，并自己接收回调；访问/刷新令牌和你确认的账户号以
AES-256-GCM 密文保存到 PostgreSQL。账户列表只显示掩码，系统不会默认选择
“第一个”账户。启动会从数据库恢复这份绑定，而非读取 `LIVE_ACCOUNT_ID`、
token 文件或 Docker secret volume。唯一保留在进程外的是用于解密数据库密文的
`AGENT_WEB_SESSION_KEY`；它不是 Robinhood 凭据，绝不能与数据库一起保存。

启动时 Kernel 仍会比较线上只读工具和已提交的 capability snapshot；任何必需
工具缺失、改名或 schema 不兼容都会拒绝启用 Provider。token、原始 Provider
payload 和账户号不会进入日志、事件 payload 或 API 响应。
