...
// ControlWarning is an operator-visible, non-actionable execution uncertainty.
// The Cockpit deliberately receives no release or retry capability with it.
...
func (s *Store) ListControlWarnings(pendingBefore, claimBefore time.Time, limit int) ([]ControlWarning, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `
		SELECT kind,id,operation_id,ledger,symbol,state,created_at,detail
		FROM (
